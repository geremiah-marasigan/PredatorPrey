...
to setup
  clear-all
  reset-ticks
  create-predators population_predator
  create-preys population_prey
  ...
  [
    ...
    set size 3
    set color blue
    set energy starting-energy-prey
  ]
  ...
  [
    ...
    set size 3
    set color red
    set energy starting-energy-predator
  ]
...
to go
  ...
  ask predators with [count preys-here > 0] [set energy (energy + (count preys-here * 5))]

  ask preys with [count predators-here > 0] [die]
  ...
  ask predators
  [
    if sex-predator? [hatch 1]
  ]

  ask patches with [pcolor = black]
  [
    set life (life + 1)
    if life = growth-rate [set pcolor green set life 0]
  ]
...
to-report sex-prey?
  report random 100 < reproduction-rate-prey
end

to-report sex-predator?
  report random 100 < reproduction-rate-predator
end
@#$#@#$#@
GRAPHICS-WINDOW
409
...
"prey" 1.0 0 -14070903 true "" "plot count preys"
...
"" ""
PENS
"default" 1.0 0 -5298144 true "" "plot sum [energy] of predators"
"pen-1" 1.0 0 -14454117 true "" "plot sum [energy] of preys"
